...
export default function ProfilePage() {
  ...
  const handleImageUpload = async (type, file) => {
    ...
    try {
      if (type === "avatar") {
        ...
        toast.info("Cover photo update coming soon");
      }
    } catch (error) {
  ...
  if (loading && !user) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-orange-50/30 dark:from-gray-900 dark:to-gray-800 py-24">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border border-orange-100 dark:border-gray-700">
                ...
                <div className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-1 mb-3">
                    ...
                    <span className="font-medium text-gray-900 dark:text-white">{user?.rating || 4.5}</span>
                    <span className="text-gray-500">({user?.totalFavorites || 0} reviews)</span>
                  </div>
                  ...
                  <div className="space-y-2 text-left bg-gray-50 dark:bg-gray-700 rounded-xl p-4 mb-4">
                    ...
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <span className="text-gray-600 dark:text-gray-400">{user?.location || 'Not provided'}</span>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                      <FaCalendarAlt className="text-orange-500" />
                      <span className="text-gray-600 dark:text-gray-400">Joined {user?.joinDate || 'Recently'}</span>
                    </div>
                  </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border border-orange-100 dark:border-gray-700">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-green-600">{user?.responseRate || 98}%</span>
                  </div>
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div 
                      className="bg-green-500 h-2 rounded-full transition-all" 
                      style={{ width: `${user?.responseRate || 98}%` }}
                    ></div>
                  </div>
                  
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 dark:text-gray-400">Avg Response Time</span>
                    <span className="font-semibold text-orange-600">{user?.responseTime || '2 hours'}</span>
                  </div>
                  
                  <div className="grid grid-cols-3 gap-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-gray-900 dark:text-white">{user?.totalListings || 0}</div>
                      <div className="text-xs text-gray-500">Listings</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-gray-900 dark:text-white">{user?.totalViews || 0}</div>
                      <div className="text-xs text-gray-500">Views</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-gray-900 dark:text-white">{user?.totalFavorites || 0}</div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden border border-orange-100 dark:border-gray-700">
              ...
              <div className="border-b border-gray-200 dark:border-gray-700">
                <div className="flex overflow-x-auto">
                  {[
                    { id: "listings", label: "My Listings", icon: FaHome, count: user?.totalListings || 0 },
                    { id: "saved", label: "Saved", icon: FaHeart, count: savedProperties?.length || 0 },
                            ...
                            <div className="flex-1">
                              <div className="flex items-start justify-between">
                                <div>
                                  ...
                                  <p className="text-orange-600 font-semibold">{formatPrice(listing.price)}</p>
                              ...
                              <div className="flex items-center gap-4 mt-2">
                                <span className={`text-xs px-2 py-1 rounded-full ${getStatusBadge(listing.status)}`}>
                                ...
                                <span className="text-xs text-gray-500 flex items-center gap-1">
                                  <FaClock size={12} />
                                  {listing.postedDate || formatDate(listing.created_at)}
                                </span>
                              </div>
                            </div>
                          </div>
                    ...
                    {!savedProperties || savedProperties.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {savedProperties.map((property) => (
                          <Link
                            key={property.id}
                            href={`/ads/${property.id}`}
                            className="bg-gray-50 dark:bg-gray-700 rounded-xl overflow-hidden hover:shadow-md transition-all group"
                          >
                            ...
                            <div className="p-4">
                              ...
                              <p className="text-orange-600 font-semibold">{formatPrice(property.price)}</p>
                              <p className="text-sm text-gray-500 flex items-center gap-1 mt-1">
                                <FaMapMarkerAlt size={12} />
                                {property.location}
                              </p>
                              <p className="text-xs text-gray-400 mt-2">
                                Saved {property.savedDate || formatDate(property.created_at)}
                              </p>
                            </div>
                          </Link>
                        ))}
                ...
                {activeTab === "stats" && (
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {[
                        { label: "Total Views", value: stats?.totalViews || user?.totalViews || 0, icon: FaEye, color: "blue" },
                        { label: "Total Favorites", value: user?.totalFavorites || 0, icon: FaHeart, color: "red" },
                        { label: "Response Rate", value: `${user?.responseRate || 98}%`, icon: FaCheckCircle, color: "green" }
                      ].map((stat) => {
                    ...
                    <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
                      ...
                      <div className="space-y-3">
                        {[
                          { label: "Profile Views", value: stats?.monthly?.views || 0, max: 2000 },
                          { label: "Inquiries", value: stats?.monthly?.inquiries || 0, max: 100 },
                          { label: "Favorites", value: stats?.monthly?.favorites || 0, max: 100 }
                        ].map((item) => (
                          <div key={item.label}>
                    ...
                    {isEditing ? (
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={handleInputChange}
                              disabled
                              className="w-full px-4 py-2 bg-gray-100 dark:bg-gray-600 border border-gray-200 dark:border-gray-600 rounded-lg cursor-not-allowed"
                            />
                      ...
                      <>
                        ...
                        <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
                          ...
                          <p className="text-sm text-gray-500">Last changed: {new Date().toLocaleDateString()}</p>
                        </div>
